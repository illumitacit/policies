...
We store the content you provide on our servers when creating Macros in your IllumiTacit account. This includes, but is
not limited to, the Inputs you define, Function Generators, Function Generator Examples, Document Indices, and
References uploaded to Document Indices. This helps you use our products as they're designed, like running prompts with
third-party language models. We keep this content for as long as the related objects are active on our server, and for
as long as your account is active. All related data is deleted within 60 days, including from backups, when the related
objects are deleted in your account. If you delete your account, we'll erase the content within 60 days.
...
For Macros using the "Local Text Generation" feature (also known as "Generated Procedures"), the content uploaded during
runtime execution is sent to our servers, but not stored, except when executed from the Microsoft Office Add-on. Unlike
standard Macros, all processing for Local Text Generation Macros will happen within our servers and are not shared with
third-party service providers.

For users of the Microsoft Office Add-on, no content is uploaded to our servers when using Macros with the "Local Text
Generation" (also known as "Functions") feature. All computations for the Macro is handled locally within the Microsoft
Word Add-on (in the browser). We do track and upload the number of words that are generated by the local generation. The
word count is calculated entirely in the extension and not on our servers.

**Document References**

We also store contents uploaded as Documents to Document Indices. This allows us to retrieve the relevant passages for
enhancing Macros. Content uploaded in this manner is also sent to our [third-party service
providers](#when-we-access-or-share-your-information) for processing. As mentioned above, our third-party service
providers may keep this content for a limited time for specific limited uses. All related data is deleted within 60
days, including from backups, when the individual Reference Document or the Document Index is deleted.

We do not store contents uploaded as Documents to Reference Inputs during Macro execution. However, content uploaded in
this manner is still sent to our third-party service providers.

**Experiments and Chat Sessions**
