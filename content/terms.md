...
4. Every generated text from a request to generate content (including, but not limited to, executing Macros, sending a
   message in a Chat based Experiments, sending a prompt in a Trial based Experiment) will be counted towards usage
   on the account. This includes those originating from any request that is canceled mid process, either by using
